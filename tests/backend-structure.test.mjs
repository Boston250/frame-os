...
test("employee password reset is available in the staff drawer",async()=>{const [client,app]=await Promise.all([readFile(new URL("../app/api-client.ts",import.meta.url),"utf8"),readFile(new URL("../app/frame-app.tsx",import.meta.url),"utf8")]);assert.match(client,/resetEmployeePassword/);assert.match(app,/Reset temporary password/);assert.match(app,/shown once/);});
